    ...
    <div class="content">
      <div class="container-fluid">
            ...
            <form @submit.prevent="updateGroup">
              <div class="form-group text-center">
                <img
                  ...
                  :class="{ 'is-invalid': !!chatError.avatar }"
              ...
              <div class="form-group">
                ...
                <textarea
                  ...
                  :class="{ 'is-invalid': !!chatError.description }"
                ></textarea>
        ...
        <CustomTablePaginated
          ...
          :data="members"
          :columns="columns"
          v-model:currentPage="memberCurrentPage"
          v-model:lastPage="memberLastPage"
          v-model:total="memberTotal"
          v-model:pageSize="memberPageSize"
          v-model:keyword="memberKeyword"
          @search-change="handleMemberSearchChange"
        />
      </div>
    </div>
  </div>
...
watch(
  ...
  async (newChatId) => {
    ...
    if (chatType.value === "group") {
      await generateChatMembers(
        memberKeyword.value,
        memberCurrentPage.value,
        memberPageSize.value,
      );
      isAdmin.value =
        members.value.some(
          (member) =>
            member.user.id === userStore.id && member.role === "admin",
        ) || false;
    }
  },
  { immediate: true },
    ...
    reader.onloadend = function () {
      ...
      img.onload = function () {
        ...
        canvas.toBlob((blob) => {
          ...
          selectedImage.value = new File([blob], "profile.png", {
            type: "image/png",
          });
  ...
  try {
    ...
    if (status === 422) {
      Object.keys(chatError).forEach((key) => {
        chatError[key] = data.errors[key] ? data.errors[key][0] : "";
      });
      return CloseModal();
    }
    return MessageModal({ icon: "error", title: "Error", text: data.message });
...
async function generateChatMembers(
  searchKeyword = "",
  page = 1,
  per_page = 25,
) {
